...
>(({ className, children, closeOnOverlayClick = true, hideClose = false, ...props }, ref) => (
	<DialogPortal>
		...
		<DialogPrimitive.Content
			ref={ref}
			className={cn(
				"fixed left-[50%] top-[50%] z-50 grid w-full max-w-full ma:max-w-lg max-ma:h-full ma:max-h-[calc((var(--vh,1vh)_*_90))] ma:rounded-lg overflow-y-auto translate-x-[-50%] translate-y-[-50%] gap-4 ma:border-2 bg-background p-6 shadow-lg duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-left-1/2 data-[state=closed]:slide-out-to-top-[48%] data-[state=open]:slide-in-from-left-1/2 data-[state=open]:slide-in-from-top-[48%] grid-cols-[1fr] content-start",
				className
			)}
			{...props}
			onInteractOutside={(e) => {
