...
import { type NewPostData, type PostData, type PostDocPopulated } from "@/interfaces/Post";
...
export async function postDocuments_toData({
  ...
  return postsFiltered
    ...
    .map((post) => ({
      ...
      url1: post.url1,
      url2: post.url2,
      dateFrom: new Date(post.dateFrom),
      dateTo: post.dateTo ? new Date(post.dateTo) : undefined,
      entryType: post.entryType,
      galleryNav: post.galleryNav as boolean,
      galleryCaptions: post.galleryCaptions as boolean,
      galleryDisplay: post.galleryDisplay as boolean,
...
export function postFormData_toNewPostData({
  ...
  return {
    ...
    galleryDisplay: data.get("galleryDisplay") as string,
